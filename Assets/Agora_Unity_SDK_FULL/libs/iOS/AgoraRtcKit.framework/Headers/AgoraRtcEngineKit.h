...
/** Occurs when the local user rejoins a channel.
 ...
 @param channel Channel name.
 @param uid     User ID. If the `uid` is specified in the [joinChannelByToken]([AgoraRtcEngineKit joinChannelByToken:channelId:info:uid:joinSuccess:]) method, the specified user ID is returned. If the user ID is not specified when the joinChannelByToken method is called, the server automatically assigns a `uid`.
 @param elapsed Time elapsed (ms) from the user calling `joinChannelByToken` until the SDK triggers this callback.
 */
...
/** Enables/Disables the super-resolution feature for a remote user's video stream. This is a beta feature.
 ...
 After calling this method, the SDK triggers the [superResolutionEnabledOfUid]([AgoraRtcEngineDelegate rtcEngine:superResolutionEnabledOfUid:enabled:reason:]) callback to report whether you have successfully enabled super resolution.
 ...
 **Warning:** The super resolution feature requires extra system resources. To balance the visual experience and system consumption, the SDK poses the following restrictions:
 ...
 - This feature can only be enabled for a single remote user; otherwise, the SDK triggers the [didOccurWarning]([AgoraRtcEngineDelegate rtcEngine:didOccurWarning:]) callback and returns the warning code `AgoraWarningCodeSuperResolutionUserCountOverLimitation(1611)`.
 - The original resolution of the remote user's video cannot exceed 640 × 360 pixels; otherwise, the SDK triggers the `didOccurWarning` callback and returns the warning code `AgoraWarningCodeSuperResolutionStreamOverLimitation(1610)`.
 - The feature cannot be enabled in certain specific devices, if one of these devices is used, the SDK triggers the `didOccurWarning` callback and returns the warning code  `AgoraWarningCodeSuperResolutionDeviceNotSupported(1612)`.
 ...
 @param mode The mode of super resolution. See AgoraVideoSRMode.
 @param uid The user ID of the remote user. This parameter only applies when `mode` is set as `AgoraVideoSRModeManual(0)`.
 ...
 */
- (int)enableRemoteSuperResolution:(BOOL)enabled mode:(AgoraVideoSRMode)mode uid:(NSUInteger)uid NS_SWIFT_NAME(enableRemoteSuperResolution(_:mode:uid:));
 ...
 **Note**
 ...
 @param sound1 The absolute path (including the filename extensions) of the file for the downbeat. For
 example: `/var/mobile/Containers/Data/audio.mp4`.
 @param sound2 The absolute path (including the filename extensions) of the file for the upbeats. For
 example: `/var/mobile/Containers/Data/audio.mp4`.
 ...
 **Note**
 ...
 - `YES`: Enables deep-learning noise reduction.
...
/** Enables the external audio sink.
 ...
 @note When using the SDK with versions earlier than v3.5.0, you will not receive the [onPlaybackAudioFrame]([AgoraAudioDataFrameProtocol onPlaybackAudioFrame:]) callback after using the Pull method to set the external audio sink.
 */
 ...
 **Note**
 ...
 - When using the SDK with versions earlier than v3.5.0, you will not receive the [onPlaybackAudioFrame]([AgoraAudioDataFrameProtocol onPlaybackAudioFrame:]) callback after using the Pull method to set the external audio sink.
 ...
 **Deprecated** This method is deprecated as of v3.7.1. Use [enableRemoteSuperResolution]([AgoraRtcEngineKit enableRemoteSuperResolution:mode:uid:]) instead.
